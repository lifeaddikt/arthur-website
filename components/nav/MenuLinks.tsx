    ...
    return collections.map(collection => {
      ...
      return (
        <Link
          ...
          className={`text-lg ${isActive ? 'underline' : ''} uppercase text-center lg:text-left py-6 lg:py-0 border-t lg:border-none ${collections[collections.length - 1].id === collection.id ? 'border-b lg:border-none' : ''}`}>
          {collection.name}
        </Link>
      )
    })
